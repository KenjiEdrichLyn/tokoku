...
Widget watchTab(List<ProductItem> datalist){
  ...
  return Padding(
    ...
    child: GridView.builder(
        ...
        itemBuilder: (BuildContext context, index){
          if(datalist[index].type == "Watch") {
            return InkWell(
              onTap: (){
                Navigator.push(context, MaterialPageRoute(
                    builder: (context) => WatchDetailScreen(object: datalist[index])
                ));
              },
              child: Container(
                decoration: BoxDecoration(
      ...
      onPressed: () {
        setState(() {
          if(_iconColor == Colors.red){
            _iconColor = Colors.black26;
            watchList[index].isLiked = false;
          }else{
            _iconColor = Colors.red;
            watchList[index].isLiked = true;
          }
        });
      },
    );
